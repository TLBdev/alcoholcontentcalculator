...
function Results(props) {
    return (<section>
        <div>{`${props.values.total} ml, ${props.values.total / 18} standard drinks(US), ${props.values.total / 10} units(UK)`}</div>
        ...
        {props.values.glass && props.values.professional ? <div>{`${props.values.total * 1.433} ml, ${(props.values.total * 1.433) / 18} standard drinks(US), ${(props.values.total * 1.433) / 10} units(UK)`}</div> : null}
